  ...
  <div class="main_body">
    ...
    <div>
      ...
      >
        ...
        <template #1>
          <base-input v-model="agingTimeout" max-len="6" @check="onlyNum('agingTimeout')" />
          <span class="tipAfterInputBox">Sec. (130-153025)</span>
        </template>
        <template #2>
          <select v-model="querierStatus">
            <option value="1">Enabled</option>
            <option value="2">Disabled</option>
          </select>
        </template>
        <template #3>
          <select v-model="fastLeaveStatus">
            <option value="1">Enabled</option>
            <option value="2">Disabled</option>
          </select>
        </template>
        ...
        <template #5>
          <base-input v-model="maxResponseTime" max-len="4" @check="onlyNum('maxResponseTime')" />
          <span class="tipAfterInputBox">Sec. (10-25)</span>
        </template>
        <template #6>
          <base-input v-model="robustness" max-len="4" @check="onlyNum('robustness')" />
          <span class="tipAfterInputBox">Times. (2-255)</span>
        </template>
        <template #7>
          <base-input v-model="lastMemberQueryInterval" max-len="4" @check="onlyNum('lastMemberQueryInterval')" />
          <span class="tipAfterInputBox">Sec. (1-25)</span>
        </template>
        <template #8>
          <base-input v-model="routerTimeout" max-len="4" @check="onlyNum('routerTimeout')" />
