<template>
  <div class="main_body">
    <div id="basetitle">MLD Snooping Router Port</div>
    <div>
      <el-table
        v-loading="loading"
        :data="pageRows"
      ...
      >
        <el-table-column prop="vlan" label="802.1Q VLAN" min-width="120" />
...
export default {
  ...
  methods: {
    ...
    openModify(row) {
      // 原版 Modify 跳转到独立编辑页(Modify MLD Static Router Port)
      this.$router.push({ path: '/network/mld-snooping/router-port-modify', query: { vlan: row.vlan }})
    }
  }
}
</script>
