<template>
  <div class="main_body">
    <div id="basetitle">MLD Snooping Settings</div>
    <div>
      <common-table
        header-title="MLD Snooping Settings"
        :first-column="['MLD Snooping Status', 'Aging Timeout', 'Querier Status', 'Fast Leave Status', 'Query Interval', 'Max Response Time', 'Robustness Variable', 'Last Member Query Interval', 'Router Timeout']"
      >
        <template #0>
      ...
      >
        <el-table-column prop="vlan" label="VLAN Index" min-width="120" />
...
export default {
  ...
  computed: {
    off() {
      // MLD Snooping 关闭时,除 Status 下拉外其余控件禁用置灰(与原版一致)
  ...
  methods: {
    load() {
      ...
      cgiGet('net_mldSettings').then(d => {
        ...
        this.lastMemberQueryInterval = String(d.lastMemberQueryInterval != null ? d.lastMemberQueryInterval : 2)
